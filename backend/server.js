...
// Middleware
app.use(cors({
  origin: 'http://185.119.59.54:5173',
  credentials: true,
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    db.run(
      'INSERT INTO users (username, password, is_admin, full_name, date_of_birth, place_of_study) VALUES (?, ?, 0, ?, ?, ?)',
...
// Запуск сервера
app.listen(PORT, () => {
  console.log(`Сервер запущен на http://185.119.59.54:${PORT}`);
});
